...
public class Ranking {
	...
	public String getRankUsuario() {
		...
		for (int i = 0; i < this.usuariosMaisPopulares.size(); i++) {
			if (i != 0)
				rank += "; ";
			rank += "(" + (i + 1) + ") ";
			rank += this.usuariosMaisPopulares.get(i).getNome() + " "
					+ this.usuariosMaisPopulares.get(i).getPopularidade();
		}
		...
		for (int i = 0; i < this.usuariosMenosPopulares.size(); i++) {
			if (i != 0)
				rank += "; ";
			rank += "(" + (i + 1) + ") ";
			rank += this.usuariosMenosPopulares.get(i).getNome() + " "
					+ this.usuariosMenosPopulares.get(i).getPopularidade();
		}
		rank += ";";
		return rank;
	}
	...
	public String getRankHashtag() {
		...
		for (int i = 0; i < this.hashtags.size(); i++) {
			if (i != 0)
				rank += "; ";
			rank += "(" + (i + 1) + ") ";
			rank += this.hashtags.get(i) + ": "
					+ frequenciaHashtag.get(this.hashtags.get(i));
		}
		rank += ";";
		return rank;
	}

	public void atualizarRank(List<Usuario> usuarios) {
		this.frequenciaHashtag.clear();
		this.hashtags.clear();
		this.usuariosMaisPopulares.clear();
		this.usuariosMenosPopulares.clear();
		for (Usuario usuario : usuarios) {
			atualizarRankUsuarioMaisPopulares(usuario);
			atualizarRankUsuarioMenosPopulares(usuario);
			for (Post post : usuario.getPosts()) {
				for (String hashtag : post.getHashtags()) {
					atualizarHashtag(hashtag);
				}
			}
		}
	...
	private void atualizarRankUsuarioMaisPopulares(Usuario usuario) {
	...
	private void atualizarRankUsuarioMenosPopulares(Usuario usuario) {
	...
	private void atualizarHashtag(String hashtag) {

		if (!frequenciaHashtag.containsKey(hashtag))
			frequenciaHashtag.put(hashtag, new Integer(1));
		else
			frequenciaHashtag.put(hashtag,
					new Integer(frequenciaHashtag.get(hashtag).intValue() + 1));

		if (!hashtags.contains(hashtag)) {
			if (hashtags.size() < 3) {
				hashtags.add(hashtag);
			} else {
				if (compararHashTag(hashtag, hashtags.get(2)) < 0) {
					hashtags.remove(2);
					hashtags.add(hashtag);
				}
			}
		...
		Collections.sort(hashtags, (h1, h2) -> compararHashTag(h1, h2));
	}

	private int compararHashTag(String o1, String o2) {
